...
const orgUrn = encodeURIComponent("urn:li:organization:112470973");

async function test() {
  // Test v2/organizationalEntityAcls (Find what organizations this user administers)
  const aclRes = await fetch("https://api.linkedin.com/v2/organizationalEntityAcls?q=roleAssignee", {
    headers: { Authorization: `Bearer ${token}` }
  });
  console.log("ACL status:", aclRes.status);
  console.log("ACL body:", await aclRes.text());

  // Test Follower stats with encoded URN
  const folRes = await fetch(`https://api.linkedin.com/rest/organizationalEntityFollowerStatistics?q=organizationalEntity&organizationalEntity=${orgUrn}`, {
    headers: {
